...
settings = dict(
    ...
    boorus=[
        ...
        {"name": "AIBooru", "host": "https://aibooru.space", "username": "", "apikey": ""},
    ],
    form_defaults=dict(
        replace_spaces=True,
...
def savesettings(active, username, apikey, negprompt, ignoredtags):
    ...
    Args:
        ...
        ignored_tags (str): Tags to ignore in each image selection
    """
    settings["active"] = active
    settings["negativeprompt"] = negprompt
    settings["ignored_tags"] = ignoredtags
...
def searchbooru(query, removeanimated, curpage, pagechange=0):
    ...
    request = Request(
        url, data=None, headers={"User-Agent": "booru2prompt, a Stable Diffusion project (made by Borderless)"}
    )
...
def on_ui_tabs():
    ...
    negprompt = gr.Textbox(
        label="Negative Prompt",
        value=settings["negativeprompt"],
        placeholder="Negative prompt to send with along with each prompt",
    )
    ignoredtags = gr.Textbox(
        label="Ignored Tags",
        value=settings["ignored_tags"],
        placeholder="Tags to ignore in each prompt (requires replacespaces and replaceunderscores)",
    )
    ...
    with gr.Blocks() as interface:
        with gr.Tab("Select"):
            with gr.Row(equal_height=True):
                with gr.Column():
                    ...
                    imagelink = gr.Textbox(
                        label="Link to image page",
                        elem_id="selectbox",
                        placeholder="https://danbooru.donmai.us/posts/4861569 or id:4861569",
                        value=settings["form_defaults"]["image_link"],
                    )

                    with gr.Row():
                        selectedtags_artist = gr.Textbox(label="Artist Tags", interactive=False)
                        includeartist = gr.Checkbox(
                            value=settings["form_defaults"]["include_artist"],
                            label="Include artist tags in tag string",
                            interactive=True,
                        )
                    with gr.Row():
                        selectedtags_character = gr.Textbox(label="Character Tags", interactive=False)
                        includecharacter = gr.Checkbox(
                            value=settings["form_defaults"]["include_character"],
                            label="Include character tags in tag string",
                            interactive=True,
                        )
                    with gr.Row():
                        selectedtags_copyright = gr.Textbox(label="Copyright Tags", interactive=False)
                        includecopyright = gr.Checkbox(
                            value=settings["form_defaults"]["include_copyright"],
                            label="Include copyright tags in tag string",
                            interactive=True,
                        )
                    with gr.Row():
                        selectedtags_meta = gr.Textbox(label="Meta Tags", interactive=False)
                        includemeta = gr.Checkbox(
                            value=settings["form_defaults"]["include_meta"],
                            label="Include meta tags in tag string",
                            interactive=True,
                        )
                    ...
                    replacespaces = gr.Checkbox(
                        value=settings["form_defaults"]["replace_spaces"],
                        label="Replace spaces with a comma and a space",
                        interactive=True,
                    )
                    replaceunderscores = gr.Checkbox(
                        value=settings["form_defaults"]["replace_underscores"], label="Replace underscores with spaces"
                    )
                ...
                with gr.Column():
                    ...
                    with gr.Row(equal_height=True):
                        ...
                        sendselected = modules.generation_parameters_copypaste.create_buttons(
        ...
        with gr.Tab("Search"):
            with gr.Row(equal_height=True):
                with gr.Column():
                    ...
                    searchtext.submit(
                        fn=searchbooru, inputs=[searchtext, removeanimated, curpage], outputs=[searchimages, curpage]
                    )
                    searchbutton.click(
                        fn=searchbooru, inputs=[searchtext, removeanimated, curpage], outputs=[searchimages, curpage]
                    )
                with gr.Column():
                    with gr.Row():
                        ...
                        prevpage.click(
                            fn=gotoprevpage,
                            inputs=[searchtext, removeanimated, curpage],
                            outputs=[searchimages, curpage],
                        )
                        nextpage.click(
                            fn=gotonextpage,
                            inputs=[searchtext, removeanimated, curpage],
        ...
        with gr.Tab("Settings/API Keys"):
            ...
            savesettingsbutton.click(fn=savesettings, inputs=[booru, username, apikey, negprompt, ignoredtags])
            booru.change(
                fn=updatesettings, inputs=booru, outputs=[username, apikey, activeboorutext1, activeboorutext2]
            )
